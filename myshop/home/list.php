...
<html>
	...
	<body bgcolor='whitesmoke'>
		...
		<div class="list-filter-category" style='width:980px;height:10px;'>
			<a class="tag curr" href="">全部着迷</a>
			<?php
			//查寻旅行的子类别
				$sql="select * from type where pid=4";
				$result=mysql_query($sql,$link);
				if(mysql_num_rows($result)>0){
					while($row=mysql_fetch_assoc($result)){
					echo "<a class='tag'  href='doselectlist.php?id={$row['id']}'>{$row['name']}</a>";
					
					}
				}	
				
			?>
			<a class="tag"  href="">手工</a>
			<a class="tag"  href="">摄影</a>
			<a class="tag"  href="">玩偶</a>
			<a class="tag"  href="">怀旧</a>
		</div>
		...
		//计算最大的页数：	
			$sql="select count(*) from goods where state=2 and typeid in ( select id from type where pid=6)";
			$result=mysql_query($sql,$link);
		...
		//拼装分页sql语句（limit【当前页-1】*页大小，【页大小】）
			$limit=" limit ".($page-1)*$pagesize.",".$pagesize;
				...
				//准备sql语句，并执行
				 $sql="select * from goods where state=2 and typeid in( select id from type where pid=6)".$limit;
				 $result=mysql_query($sql,$link);
				//释放结果集，关闭数据库连接：
	...
	//====================输出分页信息===============================	
		...
		echo "&nbsp;&nbsp;&nbsp;&nbsp;";
		echo "<a href='list.php?p=1'>首页|</a>";
		echo "<a href='list.php?p=".($page-1)."'>上一页|</a>";
		echo "<a href='list.php?p=".($page+1)."'>下一页|</a>";
		echo " <a href='list.php?p={$maxpage}'>末页</a> ";
		echo "</div>";
