...
<html>
	...
	<body bgcolor='whitesmoke'>
		...
		//计算最大的页数：	
			$sql="select count(*) from goods where state=2 and typeid in ( select id from type where pid=4)";
			$result=mysql_query($sql,$link);
		...
		//拼装分页sql语句（limit【当前页-1】*页大小，【页大小】）
			$limit=" limit ".($page-1)*$pagesize.",".$pagesize;
				...
				//准备sql语句，并执行
				 $sql="select * from goods where state=2 and typeid in( select id from type where pid=4)".$limit;
				 $result=mysql_query($sql,$link);
				//释放结果集，关闭数据库连接：
	...
	//====================输出分页信息===============================	
		...
		echo "&nbsp;&nbsp;&nbsp;&nbsp;";
		echo "<a href='lvlist.php?p=1'>首页|</a>";
		echo "<a href='lvlist.php?p=".($page-1)."'>上一页|</a>";
		echo "<a href='lvlist.php?p=".($page+1)."'>下一页|</a>";
		echo " <a href='lvlist.php?p={$maxpage}'>末页</a> ";
